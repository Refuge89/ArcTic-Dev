...
public:
    ...
    void AIUpdate()
    {
		...
		if ((_unit->GetHealthPct() <= 75 && GnomeCounter == 0) || (_unit->GetHealthPct() <= 50 && GnomeCounter == 1) || (_unit->GetHealthPct() <= 25 && GnomeCounter == 2))
        {
            ...
            for(int i = 0; i < 3; i++)
            {
                Gnome = _unit->GetMapMgr()->GetInterface()->SpawnCreature(CN_STEAMRIGGER_MECHANIC, SpawnCoords[i].mX, SpawnCoords[i].mY, SpawnCoords[i].mZ, SpawnCoords[i].mO, true, false, _unit->GetUInt32Value(UNIT_FIELD_FACTIONTEMPLATE), 50);
                if(Gnome)
                {
					Gnome->GetAIInterface()->SetUnitToFollow(_unit);
					Gnome->GetAIInterface()->SetFollowDistance(15.0f);
...
public:
		...
		if (t > RagePhaseTimer)
		{
			...
			else
			{
				if (EnrageTimer == 0 && RagePhase == 0)
				{
					...
					_unit->GetAIInterface()->MoveTo(DistillerMoveTo[DistillerNumber].mX, DistillerMoveTo[DistillerNumber].mY, DistillerMoveTo[DistillerNumber].mZ, DistillerMoveTo[DistillerNumber].mO);
	...
	Unit* GetClosestDistiller()
	{
		...
		for (int i = 1; i < 5; i++)
		{
			Unit2Check = _unit->GetMapMgr()->GetInterface()->GetCreatureNearestCoords(Distiller[i].mX, Distiller[i].mY, Distiller[i].mZ, 17954);
			if (!Unit2Check)
				continue;
