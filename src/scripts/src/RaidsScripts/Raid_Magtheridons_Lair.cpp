...
public:
	...
	void AIUpdate()
    {
		...
		if (ChannelersTable.size() < 5 && !EventStarted)
		{
			...
			{
				Channeler = NULL;
				Channeler = _unit->GetMapMgr()->GetInterface()->GetCreatureNearestCoords(Channelers[i].mX, Channelers[i].mY, Channelers[i].mZ, 17256);
				if (!Channeler)
					continue;
	...
	void OnActivate(Player*  pPlayer)
	{
		...
		if (!Magtheridon)
		{
			for (int i = 0; i < 6; i++)
			{
				...
				Magtheridon = _gameobject->GetMapMgr()->GetInterface()->GetCreatureNearestCoords(CaveInPos[i].mX, CaveInPos[i].mY, CaveInPos[i].mZ, 17257);
			}
		}
	...
	void AIUpdate()
	{
		...
		if (!Channeler)
		{
			...
			for (int i = 0; i < 5; i++)
			{
				...
				GlobalCubeTrigger = _gameobject->GetMapMgr()->GetInterface()->GetCreatureNearestCoords(CubeTriggers[i].mX, CubeTriggers[i].mY, CubeTriggers[i].mZ, 17376);
...
public:
    ...
    void OnCombatStop(Unit* mTarget)
    {
		...
		for (int i = 0; i < 6; i++)
		{
			Gate = _unit->GetMapMgr()->GetInterface()->GetGameObjectNearestCoords(Columns[i].mX, Columns[i].mY, Columns[i].mZ, 184634+i);
			if (Gate)
				Gate->SetState(1);
		}
	...
	void PhaseTwo()
	{
		...
		if (timer_quake > 27)
		{
			if (timer_quake < 33)
			{
				...
				for (int i = 0; i < 6; i++)
				{
					Unit* Trigger = _unit->GetMapMgr()->GetInterface()->GetCreatureNearestCoords(CaveInPos[i].mX, CaveInPos[i].mY, CaveInPos[i].mZ, 17474);
					if (Trigger)
					{
		...
		if(timer_caveIn && (timer_caveIn != 1 || (_unit->GetCurrentSpell() == NULL && timer_caveIn == 1 && !aura)))
		{
			...
			if (timer_caveIn == 5)
			{
				for (int i = 0; i < 6; i++)
				{
					...
					if (Trigger)
					{
						Trigger->CastSpellAoF(CaveInPos[i].mX, CaveInPos[i].mY, CaveInPos[i].mZ, spells[5].info, spells[5].instant);
					}
				}
