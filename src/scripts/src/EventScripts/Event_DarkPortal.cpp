...
OutlandScript::OutlandScript( MapMgr* pMapMgr ) : ArcTicInstanceScript( pMapMgr )
{
...
DarkPortalEventTriggerAI::DarkPortalEventTriggerAI( Creature* pCreature ) : ArcTicScriptCreatureAI( pCreature )
{
...
void DarkPortalEventTriggerAI::AIUpdate()
{
	...
	if ( mResurrect && !mResurrected )
	{
		...
		//	mMelgr->GetUnit()->ClearTag(); // do7me check clear tag here
...
bool DarkPortalEventTriggerAI::IsRegimentFull( AIContainer pVectorSet, size_t pSetSize )
{
	...
	ArcTicScriptCreatureAI* SetUnit = NULL;
...
void DarkPortalEventTriggerAI::PitCommanderAttackCmd()
{
	...
	ArcTicScriptCreatureAI* ScriptAI = NULL;
...
void DarkPortalEventTriggerAI::PitCommanderSpawn()
{
	...
	for ( uint32 i = 0; i < 10; i += WaveSize )
	{
		...
		ArcTicScriptCreatureAI* ScriptAI = SpawnCreature( Entry, DSPoints[ i ].mX, DSPoints[ i ].mY, DSPoints[ i ].mZ, DSPoints[ i ].mO );
		if ( ScriptAI != NULL )
		{
			// Crappy method
			switch ( Entry )
...
MelGrommHighmountainAI::MelGrommHighmountainAI( Creature* pCreature ) : ArcTicScriptCreatureAI( pCreature )
{
...
OrgrimmarGruntAI::OrgrimmarGruntAI( Creature* pCreature ) : ArcTicScriptCreatureAI( pCreature )
{
...
OrgrimmarShamanAI::OrgrimmarShamanAI( Creature* pCreature ) : ArcTicScriptCreatureAI( pCreature )
{
...
UndercityMageAI::UndercityMageAI( Creature* pCreature ) : ArcTicScriptCreatureAI( pCreature )
{
...
StormwindSoldierAI::StormwindSoldierAI( Creature* pCreature ) : ArcTicScriptCreatureAI( pCreature )
{
...
IronforgePaladinAI::IronforgePaladinAI( Creature* pCreature ) : ArcTicScriptCreatureAI( pCreature )
{
...
DarnassianArcherAI::DarnassianArcherAI( Creature* pCreature ) : ArcTicScriptCreatureAI( pCreature )
{
...
StormwindMageAI::StormwindMageAI( Creature* pCreature ) : ArcTicScriptCreatureAI( pCreature )
{
...
PitCommanderAI::PitCommanderAI( Creature* pCreature ) : ArcTicScriptCreatureAI( pCreature )
{
...
FelSoldierAI::FelSoldierAI( Creature* pCreature ) : ArcTicScriptCreatureAI( pCreature )
{
...
void FelSoldierAI::Destroy()
{
	if ( mTriggerAI != NULL )
	{
		AIMap::iterator Iter = mTriggerAI->mQueuedDemonSet.find( static_cast< ArcTicScriptCreatureAI* >( this ) );
		if ( Iter != mTriggerAI->mQueuedDemonSet.end() )
			mTriggerAI->mQueuedDemonSet.erase( Iter );

		Iter = mTriggerAI->mFightingDemonSet.find( static_cast< ArcTicScriptCreatureAI* >( this ) );
...
WrathMasterAI::WrathMasterAI( Creature* pCreature ) : ArcTicScriptCreatureAI( pCreature )
{
	SetWieldWeapon( true );
