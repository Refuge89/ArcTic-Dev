...
//	{ "EventCastSpell", &luaGameObject_EventCastSpell },
	{ "RegisterAIUpdateEvent", &luaGameObject_RegisterAIUpdate },
	{ "ModifyAIUpdateEvent", &luaGameObject_ModAIUpdate },
	{ "RemoveAIUpdateEvnet", &luaGameObject_RemoveAIUpdate },
...
public:
	...
	void OnDamageTaken(Unit* mAttacker, float fAmount)
	{
		if( m_binding->Functions[CREATURE_EVENT_ON_DAMAGE_TAKEN] != NULL )
			g_engine->OnUnitEvent( _unit, m_binding->Functions[CREATURE_EVENT_ON_DAMAGE_TAKEN], CREATURE_EVENT_ON_DAMAGE_TAKEN, mAttacker, 0, 0, fAmount, NULL, NULL);
	}
...
int luaUnit_GetName(lua_State * L, Unit * ptr)
{
	...
	{
	case TYPEID_UNIT:
		lua_pushstring(L, ((Creature*)ptr)->GetCreatureInfo() ? ((Creature*)ptr)->GetCreatureInfo()->Name : "Inconnu");
		break;
...
int luaUnit_GetSoulLinkedWith(lua_State * L, Unit * ptr)
{
	CHECK_TYPEID(TYPEID_UNIT)
		...
		if (!ptr->GetAIInterface()->isSoulLinked)
			lua_pushnil(L);
		else
			Lunar<Unit>::push(L,ptr->GetAIInterface()->soullinkedWith,false);
	return 1;
}
int luaUnit_SetSoulLinkedWith(lua_State * L, Unit * ptr)
{
	...
	if (!target || ptr->GetAIInterface()->isSoulLinked || target==ptr)
		return 0;
	else
...
int luaUnit_SetPowerType(lua_State * L, Unit * ptr)
{
	...
	if( stricmp(message, "health") == 0 )
		...
		ptr->SetPowerType(POWER_TYPE_RUNIC);
	return 1;
}
...
int luaUnit_CreateGuardian(lua_State * L, Unit * ptr)
{
	...
	if(!ptr||!guardian_entry||!lvl)
		return 0;
	ptr->CreateTemporaryGuardian(guardian_entry,duration,angle,lvl,NULL);
	return 0;
}
...
int luaUnit_ResurrectPlayer(lua_State * L, Unit * ptr)
{
	CHECK_TYPEID(TYPEID_PLAYER);
	if(ptr)
		static_cast< Player * > ( ptr ) ->ResurrectPlayer(NULL);	// DO7ME CHECK HERE
	return 0;
}
...
int luaUnit_TaxiStart(lua_State * L, Unit * ptr)
{
	CHECK_TYPEID(TYPEID_PLAYER);
	TaxiPath * path = sTaxiMgr.GetTaxiPath( luaL_checkint(L, 1) );
	int node = luaL_checkint(L, 2);
	int unk = luaL_checkint(L, 3);

	((Player*)ptr)->TaxiStart( path, node, unk );
...
int luaGameObject_SpawnGameObject(lua_State * L, GameObject * ptr)
{
	...
	//	sEventMgr.AddEvent(pC, &GameObject::ExpireAndDelete(duration);
		sEventMgr.ModifyEventTimeLeft(pC, EVENT_GAMEOBJECT_EXPIRE, duration);
	}
