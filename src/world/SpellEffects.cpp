...
void Spell::SpellEffectSummon(uint32 i)
{
	...
	switch(spe->type)
	{
		case SUMMON_PROP_TYPE_NONE:
		case SUMMON_PROP_TYPE_CONSTRUCT:
		case SUMMON_PROP_TYPE_OPPONENT:
		...
		case SUMMON_PROP_TYPE_PET:
			SpellEffectSummonTemporaryPet(i, spe, cp, v);
			return;

		case SUMMON_PROP_TYPE_GUARDIAN:
		case SUMMON_PROP_TYPE_MINION:
		case SUMMON_PROP_TYPE_RUNEBLADE:
			SpellEffectSummonGuardian(i, spe, cp, v);
			return;

		case SUMMON_PROP_TYPE_VEHICLE:
		case SUMMON_PROP_TYPE_MOUNT:
			SpellEffectSummonVehicle( i, spe, cp, v );
			return;
		...
		case SUMMON_PROP_TYPE_LIGHTWELL:
			SpellEffectSummonGuardian(i, spe, cp, v);
			return;
		case SUMMON_PROP_TYPE_COMPANION:
	...
	sLog.outError("Unknown summon type in summon property %u in spell %u %s", summonpropid, m_spellInfo->Id, m_spellInfo->Name);
}
...
/*void Spell::SpellEffectSummon(uint32 i)
{
	...
	spe = dbcSummonProps.LookupEntry( m_spellInfo->EffectMiscValueB[i] );
	if( spe == NULL )
		return;
...
void Spell::SpellEffectSummonWild(uint32 i, SummonPropertiesEntry* spe, CreatureProto* proto, LocationVector & v)
{
	...
	for(int32 j = 0; j < damage; j++)
	{
		...
		v.x = v.x + (GetDBCCastTime(i) * (cosf(followangle + v.o)));
		v.y = v.y + (GetDBCCastTime(i) * (sinf(followangle + v.o)));
...
void Spell::SpellEffectSummonTotem(uint32 i, SummonPropertiesEntry* spe, CreatureProto* proto, LocationVector & v)
{
	...
	float landh = u_caster->GetMapMgr()->GetLandHeight(v.x, v.y);
	...
	if(s == NULL)
		return;
	s->Load(proto, u_caster, v, m_spellInfo->Id, spe->slot - 1);
	s->SetUInt32Value(UNIT_FIELD_MAXHEALTH, damage);
	s->SetUInt32Value(UNIT_FIELD_HEALTH, damage);
	s->PushToWorld(u_caster->GetMapMgr());
...
void Spell::SpellEffectSummonPossessed(uint32 i, SummonPropertiesEntry* spe, CreatureProto* proto, LocationVector & v)
{
	...
	s->Root();
	s->Phase(PHASE_SET,p_caster->GetPhase());
...
void Spell::SpellEffectSummonCompanion(uint32 i, SummonPropertiesEntry* spe, CreatureProto* proto, LocationVector & v)
{
	...
	if(u_caster->GetUInt64Value(UNIT_FIELD_CRITTER) != 0)
	{
		Unit* critter = u_caster->GetMapMgr()->GetUnit(u_caster->GetUInt64Value(UNIT_FIELD_CRITTER));
		Creature* c = static_cast< Creature* >(critter);
		uint32 currententry = c->GetCreatureInfo()->Id;

		c->RemoveFromWorld(false, true);
		u_caster->SetUInt64Value(UNIT_FIELD_CRITTER,0);
	...
	s->Load(proto, u_caster, v, m_spellInfo->Id, spe->slot - 1);
	s->SetUInt32Value(UNIT_CREATED_BY_SPELL, m_spellInfo->Id);
	s->GetAIInterface()->SetFollowDistance(GetDBCCastTime(i));
	s->PushToWorld(u_caster->GetMapMgr());
	u_caster->SetUInt64Value(UNIT_FIELD_CRITTER, s->GetGUID());
}
	...
	/*	default://not profession
		{
			...
			if( gameObjTarget->GetByte(GAMEOBJECT_BYTES_1, 1) == GAMEOBJECT_TYPE_GOOBER)
			{
 			//	CALL_GO_SCRIPT_EVENT(gameObjTarget, OnActivate)(TO_PLAYER(p_caster)); // Major Fix
 				CALL_GO_SCRIPT_EVENT(gameObjTarget, OnActivate)(p_caster);
				CALL_INSTANCE_SCRIPT_EVENT( gameObjTarget->GetMapMgr(), OnGameObjectActivate )( gameObjTarget, p_caster );
			};
		...
		default://not profession
			{
				...
				if(gameObjTarget->m_loot.items.size() == 0)
				{
					...
					//	lootmgr.FillGOLoot(&gameObjTarget->loot, gameObjTarget->GetInfo()->sound1,  gameObjTarget->GetMapMgr()->iInstanceMode);
						...
						lootmgr.FillGOLoot(&gameObjTarget->m_loot, gameObjTarget->GetInfo()->sound1,  0);
				}
				loottype = LOOT_CORPSE ;
			}
			break;
