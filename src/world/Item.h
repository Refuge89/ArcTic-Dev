...
public:
	...
	ARCTIC_INLINE bool IsSoulbound()
	{
	...
	ARCTIC_INLINE time_t GetEnchantmentApplytime( uint32 slot )
	{
	...
	ARCTIC_INLINE void SetCount( uint32 amt ) { SetUInt32Value( ITEM_FIELD_STACK_COUNT, amt ); }
	ARCTIC_INLINE uint32 GetCount() { return GetUInt32Value( ITEM_FIELD_STACK_COUNT ); }
	ARCTIC_INLINE void SetDurability( uint32 Value ) { SetUInt32Value(ITEM_FIELD_DURABILITY, Value ); };
	ARCTIC_INLINE void SetDurabilityToMax() { SetUInt32Value( ITEM_FIELD_DURABILITY, GetUInt32Value( ITEM_FIELD_MAXDURABILITY ) ); }
	...
	ARCTIC_INLINE uint32 GetItemRandomSuffixFactor() { return m_uint32Values[ITEM_FIELD_PROPERTY_SEED]; }
	static uint32 GenerateRandomSuffixFactor( ItemPrototype* m_itemProto );

	ARCTIC_INLINE void SetRandomProperty( uint32 id )
	{
		SetUInt32Value( ITEM_FIELD_RANDOM_PROPERTIES_ID, id );
		random_prop = id;
	}

	ARCTIC_INLINE void SetRandomSuffix( uint32 id )
	{
		int32 r_id = -(int32(id));
