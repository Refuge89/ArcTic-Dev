...
public:
	...
	ARCTIC_INLINE size_t GetSessionCount() const { return m_sessions.size(); }

	ARCTIC_INLINE size_t GetQueueCount() { return mQueuedSessions.size(); }
	void GetStats(uint32 * GMCount, float * AverageLatency);

	ARCTIC_INLINE uint32 GetPlayerLimit() const { return m_playerLimit; }
	void SetPlayerLimit(uint32 limit) { m_playerLimit = limit; }

	ARCTIC_INLINE bool getAllowMovement() const { return m_allowMovement; }
	void SetAllowMovement(bool allow) { m_allowMovement = allow; }
	ARCTIC_INLINE bool getGMTicketStatus() { return m_gmTicketSystem; };
	...
	ARCTIC_INLINE float getRate(int index)
	{
	...
	ARCTIC_INLINE void setIntRate(int index, uint32 value)
	{
	...
	ARCTIC_INLINE uint32 GetTimeOut(){return TimeOut;}
