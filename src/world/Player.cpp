...
ARCTIC_INLINE uint32 GetSpellForLanguage(uint32 SkillID)
{
	switch(SkillID)
	{
...
void Player::DespawnPet()
{
	if( m_Summon == NULL || !m_Summon->IsPet())
		return;
...
void Player::UpdateNearbyGameObjects()
{
	...
	for (Object::InRangeSet::iterator itr = GetInRangeSetBegin(); itr != GetInRangeSetEnd(); ++itr)
	{
		if((*itr)->GetTypeId() == TYPEID_GAMEOBJECT)
		{
			...
			if((*itr)->GetByte(GAMEOBJECT_BYTES_1, GAMEOBJECT_BYTES_TYPE_ID) == GAMEOBJECT_TYPE_QUESTGIVER)
			{
...
void Player::Possess(Unit* pTarget, uint32 delay)
{
	...
	if(delay != 0)
	{
		sEventMgr.AddEvent(this, &Player::Possess, pTarget, uint32(0), 0, delay, 1, 0);
		return;
	}
	...
	if(!IsPet() || !IsVehicle() )
	{
		TO_CREATURE(pTarget)->BuildPetSpellList(data);
	}
...
void Player::UnPossess()
{
	...
	if(!pTarget->IsPet() && (pTarget->GetUInt64Value(UNIT_FIELD_CREATEDBY) == GetGUID()))
	{
...
void Player::Die(Unit* pAttacker, uint32 damage, uint32 spellid)
{
	...
	if(GetUInt64Value(UNIT_FIELD_CHANNEL_OBJECT) != 0)
	{
		...
		if(spl != NULL)
		{
			...
			for(int i = 0; i < 3; i++)
			{
				...
				{
					uint64 guid = GetUInt64Value(UNIT_FIELD_CHANNEL_OBJECT);
			...
			if(spl->GetSpellProto()->ChannelInterruptFlags == 48140) spl->cancel();
		}
	}
...
void Player::HandleSpellLoot(uint32 itemid)
{
	...
	for(itr = loot.items.begin(); itr != loot.items.end(); ++itr)
	{
		...
		m_ItemInterface->CanReceiveItem(ItemPrototypeStorage.LookupEntry(looteditemid), count, 0);
	}
}
