...
//	void CreateAsSummon(uint32 entry, CreatureInfo *ci, Creature* created_from_creature, Unit* owner, SpellEntry *created_by_spell, uint32 type, uint32 expiretime);
	bool CreateAsSummon(uint32 entry, CreatureInfo *ci, Creature* created_from_creature, Unit* owner, SpellEntry *created_by_spell, uint32 type, uint32 expiretime, LocationVector* Vec = NULL, bool dismiss_old_pet = true);
	...
	ARCTIC_INLINE void SetPetAction(uint32 act) { m_Action = act; }
	ARCTIC_INLINE uint32 GetPetAction(void) { return m_Action; }

	ARCTIC_INLINE void SetPetState(uint32 state) { m_State = state; }
	ARCTIC_INLINE uint32 GetPetState(void) { return m_State; }

	ARCTIC_INLINE void SetPetDiet(uint32 diet) { m_Diet = diet; }
	ARCTIC_INLINE void SetPetDiet() { m_Diet = myFamily->petdietflags; }

	ARCTIC_INLINE uint32 GetPetDiet(void) { return m_Diet; }
	
	ARCTIC_INLINE AI_Spell* GetAISpellForSpellId(uint32 spellid)
	{
	...
	ARCTIC_INLINE void ClearPetOwner() { m_Owner = NULL; }
	...
	ARCTIC_INLINE void RemoveSpell(uint32 SpellID)
	{
		SpellEntry * sp = dbcSpell.LookupEntry(SpellID);
		if(sp)
			RemoveSpell(sp);
	}
	ARCTIC_INLINE void SetSpellState(uint32 SpellID, uint16 State)
	{
		SpellEntry * sp = dbcSpell.LookupEntry(SpellID);
		if(sp) SetSpellState(sp, State);
	}
	ARCTIC_INLINE uint16 GetSpellState(uint32 SpellID)
	{
		if(SpellID == 0)
