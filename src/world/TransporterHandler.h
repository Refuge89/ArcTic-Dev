...
public:
	...
	ARCTIC_INLINE void SetLength(const unsigned int sz)
	{
		i_nodes.resize( sz );
	}

	ARCTIC_INLINE size_t Size(void) const { return i_nodes.size(); }
	ARCTIC_INLINE void Resize(unsigned int sz) { i_nodes.resize(sz); }
	ARCTIC_INLINE void Clear(void) { i_nodes.clear(); }
	ARCTIC_INLINE PathNode* GetNodes(void) { return static_cast<PathNode *>(&i_nodes[0]); }
