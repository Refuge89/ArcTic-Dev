...
void TotemSummon::Load(CreatureProto* proto, Unit* owner, LocationVector & position, uint32 spellid, int32 summonslot)
{
	...
	// Set up the creature.
	SetUInt32Value(UNIT_FIELD_MAXPOWER1 + POWER_TYPE_FOCUS, owner->getLevel() * 30);
	SetPower(POWER_TYPE_FOCUS, owner->getLevel() * 30);
	SetUInt32Value(GAMEOBJECT_LEVEL, owner->getLevel());
	setRace(0);
	setClass(1);
	setGender(2);
	SetPowerType(1);
	SetUInt32Value(UNIT_FIELD_BASEATTACKTIME + MELEE, 2000);
	SetUInt32Value(UNIT_FIELD_BASEATTACKTIME + OFFHAND, 2000);
	...
	for(uint8 school = 0; school < SCHOOL_ALL; school++)
	{
...
void TotemSummon::SetupSpells()
{
	...
	SpellEntry* creatorspell = dbcSpell.LookupEntry(GetUInt32Value(UNIT_CREATED_BY_SPELL));
//	SpellEntry* TotemSpell = dbcSpell.LookupEntry(proto->AISpells[ 0 ]);
	SpellEntry* TotemSpell = dbcSpell.LookupEntry( 0 );

	if(TotemSpell == NULL)
	{
		sLog.outDebug("Totem %u does not have any spells to cast", creature_info->Id);
		return;
	}
	...
	if(!castingtotem)
	{
		...
		Spell* pSpell = new Spell(this, TotemSpell, true, 0);
