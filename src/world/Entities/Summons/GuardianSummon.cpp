...
void GuardianSummon::Load(CreatureProto* proto, Unit* owner, LocationVector & position, uint32 spellid, int32 summonslot)
{
	...
	SetUInt32Value(UNIT_FIELD_MAXPOWER1 + POWER_TYPE_MANA, GetUInt32Value(UNIT_FIELD_MAXPOWER1 + POWER_TYPE_MANA) + 28 + 10 * getLevel());
	SetPower(POWER_TYPE_MANA, GetUInt32Value(UNIT_FIELD_POWER1 + POWER_TYPE_MANA) + 28 + 10 * getLevel());
	SetUInt32Value(GAMEOBJECT_LEVEL, owner->getLevel());
	SetUInt32Value(UNIT_FIELD_MAXHEALTH, GetMaxHealth() + 28 + 30 * getLevel());
	SetUInt32Value(UNIT_FIELD_HEALTH, GetMaxHealth());
	SetByte(GAMEOBJECT_BYTES_1, 1, CREATURE_TYPE_GUARDIAN);
