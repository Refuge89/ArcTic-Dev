...
public:
	...
	void SaveToDB();
	void DeleteFromDB();
	ARCTIC_INLINE void SetCorpseState(uint32 state) { m_state = state; }
	ARCTIC_INLINE uint32 GetCorpseState() { return m_state; }
	void Despawn();
	
	ARCTIC_INLINE void SetLoadedFromDB(bool value) { _loadedfromdb = value; }
	ARCTIC_INLINE bool GetLoadedFromDB(void) { return _loadedfromdb; }
