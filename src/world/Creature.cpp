...
void Creature::Die(Unit* pAttacker, uint32 damage, uint32 spellid)
{
	...
	if((GetUInt64Value(UNIT_FIELD_CREATEDBY) == 0) && (GetTaggerGUID() != 0))
	{
	...
	if(GetUInt64Value(UNIT_FIELD_CHARMEDBY))
	{
		//remove owner warlock soul link from caster
		Unit* owner = GetMapMgr()->GetUnit(GetUInt64Value(UNIT_FIELD_CHARMEDBY));

		if(owner != NULL && owner->IsPlayer())
			TO_PLAYER(owner)->DespawnPet();
	}

	if(GetUInt64Value(UNIT_FIELD_CHARMEDBY) != 0)
	{
		Unit* charmer = m_mapMgr->GetUnit(GetUInt64Value(UNIT_FIELD_CHARMEDBY));
		if(charmer != NULL)
			charmer->UnPossess();
	}
...
void Creature::BuildPetSpellList(WorldPacket & data)
{
	...
	for(uint32 i = 0; i < 10; ++i)
	{
		...
		{
			uint32 spell = *itr;
			data << uint32( MAKE_ACTION_BUTTON( spell, i + 8 ) );
			++itr;
		}
		else
		{
