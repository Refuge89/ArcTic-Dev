...
const char * gCreatureProtoFormat             = "uuuuuucufuuuffuffuuuuuuuuuuuffsuibuufffuuuuuuuuuuuuiuc";
	...
	while(!itr->AtEnd())
	{
		cn = itr->Get();
			...
			//	SpellEntry *sp = dbcSpell.LookupEntryForced( cn->AISpells[ i ] );
				SpellEntry *sp = dbcSpell.LookupEntry( cn->AISpells[ i ] );
				if( sp == NULL )
					continue;
			...
			if( cn->spelldataid != 0 ){
				...
				//	SpellEntry *sp = dbcSpell.LookupEntryForced( spe->Spells[ i ] );
					SpellEntry *sp = dbcSpell.LookupEntry( spe->Spells[ i ] );
					if( sp == NULL )
						continue;
