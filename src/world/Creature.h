...
public:
	...
	ARCTIC_INLINE bool HasItems() { return ((m_SellItems != NULL) ? true : false); }
	...
	ARCTIC_INLINE void GetSellItemBySlot(uint32 slot, CreatureItem &ci)
	{
	...
	ARCTIC_INLINE std::vector<CreatureItem>::iterator GetSellItemBegin() { return m_SellItems->begin(); }
	ARCTIC_INLINE std::vector<CreatureItem>::iterator GetSellItemEnd()   { return m_SellItems->end(); }
	...
	ARCTIC_INLINE void SetSheatheForAttackType(uint8 type) { SetByte(UNIT_FIELD_BYTES_2, 0, type); }
	ARCTIC_INLINE bool isQuestGiver() { return HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_QUESTGIVER ); };
	...
	ARCTIC_INLINE CreatureAIScript * GetScript() { return _myScriptClass; }
	void LoadScript();
