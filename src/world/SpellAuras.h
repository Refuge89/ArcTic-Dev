...
public:
    ...
    ARCTIC_INLINE SpellEntry* GetSpellProto() const { return m_spellProto; }
    ARCTIC_INLINE uint32 GetSpellId() const {return m_spellProto->Id; }
    ARCTIC_INLINE bool IsPassive(){ if(!m_spellProto) return false; return (m_spellProto->Attributes & ATTRIBUTES_PASSIVE && !m_areaAura);}

    ARCTIC_INLINE int32 GetDuration() const { return m_duration; }
    void SetDuration(int32 duration) { m_duration = duration; }
	void SetTimeLeft(int32 time);

	ARCTIC_INLINE uint32 GetModCount() const { return m_modcount; }
	ARCTIC_INLINE Modifier *GetMod(uint32 x) { return &m_modList[x]; }

    ARCTIC_INLINE uint8 GetAuraSlot() const { return m_auraSlot; }
	void SetAuraSlot(uint8 slot) { m_auraSlot = slot; }
	ARCTIC_INLINE uint8 GetAuraFlags() const { return m_auraFlags; }
	void SetAuraFlags(uint8 flags) { m_auraFlags = flags; }
	void SetAuraFlag(uint16 flag) { if( ! (m_auraFlags & flag ) ) m_auraFlags |= flag; };
	ARCTIC_INLINE uint8 GetAuraLevel() const { return m_auraLevel; }
	void SetAuraLevel(uint8 level) { m_auraLevel = level; }

	ARCTIC_INLINE bool IsPositive() { return m_positive>0; }
	...
	ARCTIC_INLINE Object* GetTarget() {return ((m_target != NULL && m_target->IsInWorld()) ? TO_OBJECT(m_target): NULL);}
	...
	ARCTIC_INLINE bool HasMechanic(uint32 MechanicsType)
	{
...
public:
	...
	ARCTIC_INLINE bool IsInterrupted() { return ( m_interrupted >= 0 ); }
};
